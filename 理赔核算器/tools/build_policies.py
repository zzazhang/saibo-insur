...
ZURICH = {
    ...
    "settlement": {
        ...
        "sublimits": [
            {"id": "SL-SE", "name": "社会工程学诈骗", "items": ["F1-10"],
             "limit": 2000000, "shares_aggregate": True,
             "clause": "条款1.16–1.18 / 5.3.6 社工盗窃分项限额属主险保额一部分",
             "note": "须列在事故响应组之前，且事故响应组不得用 F1 前缀通配，"
                     "否则 F1-10 会同时命中两组而被重复封顶"},
            {"id": "SL-IR", "name": "事故响应费用",
             "items": ["F1-01", "F1-02", "F1-03", "F1-04", "F1-05", "F1-06",
                       "F1-07", "F1-08", "F1-09", "F1-11", "F4", "F5", "F6"],
             "limit": 3000000, "shares_aggregate": True,
             "clause": "条款5.2 / 1.7（显式枚举以排除 F1-10，见 SL-SE）"},
    ...
    "settlement_notes": [
        ...
        "**社工诈骗的科目借用与框架缺口**：条款 1.16 承保「雇工受欺骗主动向虚假账户"
        "转账造成的企业自身账户资金减少」，即被骗资金**本金**可赔。但本工具的 69 项科目中"
        "只有 F1-10「资金追回**费**」，没有「被骗或被盗资金本身」的位置——"
        "追回资金花的钱与丢掉的钱是两回事。案例中暂借 F1-10 承载本金加追回费，"
        "并已为其单设 SL-SE 分项组以匹配条款 5.3.6 的独立限额；"
        "**建议在 S2 大类下增设「S2-06 资金损失（被骗或被盗资金）」**。"
        "这是继行政罚款（见安达）之后，本框架的第二个已识别缺口。",
